...
def main():
    parser = argparse.ArgumentParser(description="Run the minimal 4-group LoRA / LoRA-XS ablation.")
    ...
    output_root = (
        ...
        if args.output_root
        else THIS_DIR / "eval_compare" / args.task_name / "minimal_low_rank_ablation"
    )
    ...
    if args.classifier_learning_rate is not None:
        common.extend(["--classifier_learning_rate", str(args.classifier_learning_rate)])

    groups = [
        ("plain_lora", [sys.executable, "run_glue_plain_lora_train.py", *common]),
        ("plain_loraxs", [sys.executable, "run_glue_plain_loraxs_train.py", *common]),
        (
            "mpc_lora",
            [sys.executable, "run_glue_private_loraxs_lora_train.py", *common, "--reuse_profile"],
        ),
        (
            "mpc_loraxs",
            [sys.executable, "run_glue_private_loraxs_train.py", *common, "--reuse_profile"],
        ),
    ]
    ...
    summary_json_path = output_root / "minimal_ablation_summary.json"
    with summary_json_path.open("w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2)

    summary_tsv_path = output_root / "minimal_ablation_summary.tsv"
    header = [
        "group",
        "train_steps",
        "eval_metric",
