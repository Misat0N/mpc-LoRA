...
This stage-1 script aligns the LoRA setup with the GLUE LoRA defaults used in
the LoRA-XS repository, while reusing newSHAFT's existing CrypTen private
training loop.

Key defaults:
- BERT sequence classification (`bert-base-uncased`, `sst2`)
- LoRA targets: query / value / attention.output.dense / output.dense
- LoRA dropout = 0.0
- Only finetuned params stay encrypted by default:
  LoRA weights + classifier head
- Pretrained backbone weights are public
"""
...
def _set_lora_and_classifier_trainable(model, train_classifier_head=True):
    """
    Keep LoRA params and classifier trainable.

    The base script forces `freeze_classifier_head=True` when
    `--public_non_lora_weights` is enabled. For this LoRA-XS-style LoRA stage we
    still want the classifier head to remain part of the encrypted trainable set,
    so we intentionally ignore the boolean gate here.
    """
