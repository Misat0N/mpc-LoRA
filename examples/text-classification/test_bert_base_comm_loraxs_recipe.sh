...
RUN_TAG="${RUN_TAG:-loraxs_recipe_$(date +%Y%m%d_%H%M%S)}"
...
echo "[loraxs-recipe] output_dir=${OUT_DIR}"

python run_glue_private_loraxs_recipe_train.py \
...
echo "[loraxs-recipe] done: ${OUT_DIR}"
