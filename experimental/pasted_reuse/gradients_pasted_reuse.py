...
# STRUCTURAL-REUSE: this pasted variant does not use local tag / anchor context.
# LoRA reuse is expressed only by explicit grouped matmul calls.
logger = logging.getLogger(__name__)
_CAPABILITY_LOGGED = False
_STRUCTURAL_BEAVER_PROTOCOL = None
...
def _get_structural_beaver_protocol():
    # STRUCTURAL-FB-REUSE: resolve the Beaver implementation once at module
    # scope. The runtime token variant now lives in beaver.py, so retrying a
    # missing overlay import on every hot-path call only adds overhead.
    global _STRUCTURAL_BEAVER_PROTOCOL
    if _STRUCTURAL_BEAVER_PROTOCOL is None:
        from crypten.mpc.primitives import beaver as beaver_protocol

        _STRUCTURAL_BEAVER_PROTOCOL = beaver_protocol
    return _STRUCTURAL_BEAVER_PROTOCOL


def _log_capability_once():
    # STRUCTURAL-FB-REUSE: log once so experiments can tell whether provider
    # support is active or if execution silently falls back to regular matmul.
    global _CAPABILITY_LOGGED
    if _CAPABILITY_LOGGED:
        return
    _CAPABILITY_LOGGED = True
    beaver_protocol = _get_structural_beaver_protocol()
    if hasattr(beaver_protocol, "structural_reuse_capability"):
        logger.info(
            "[structural-fb-reuse] capability=%s",
            beaver_protocol.structural_reuse_capability(),
        )
    else:
        provider = crypten.mpc.get_default_provider()
        logger.warning(
            "[structural-fb-reuse] token beaver API unavailable; "
            "provider=%s fallback_to_regular_matmul=True",
            getattr(provider, "NAME", type(provider).__name__),
        )


def get_structural_reuse_stats(reset=False):
    # STRUCTURAL-FB-REUSE: public experiment hook for checking actual token use.
    beaver_protocol = _get_structural_beaver_protocol()
    if hasattr(beaver_protocol, "get_structural_reuse_stats"):
        return beaver_protocol.get_structural_reuse_stats(reset=reset)
    return {"token_beaver_api_available": False}


def structural_reuse_capability():
    # STRUCTURAL-FB-REUSE: public experiment hook for provider compatibility.
    beaver_protocol = _get_structural_beaver_protocol()
    if hasattr(beaver_protocol, "structural_reuse_capability"):
        return beaver_protocol.structural_reuse_capability()
    provider = crypten.mpc.get_default_provider()
    return {
        "provider": getattr(provider, "NAME", type(provider).__name__),
        "token_beaver_api_available": False,
        "can_use_token_reuse": False,
    }


def _regular_grouped_matmuls(left, grouped_args):
    return [
        (left.t() if transpose_left else left).matmul(right)
        for right, transpose_left in grouped_args
    ]


def _wrap_raw_matmul_result(left_view, right, raw_result):
    result = left_view.clone()
    result.share.set_(raw_result.share.data)
    result.ptype = left_view.ptype

    if left_view.encoder.scale > 1 and right.encoder.scale > 1:
        result = result.div_(result.encoder.scale)
    elif left_view.encoder.scale > 1:
        result.encoder = left_view.encoder
    else:
        result.encoder = right.encoder
    return result


def _private_2d_arithmetic_tensor(tensor):
    try:
        ...
        return False
    return (
        isinstance(tensor, MPCTensor)
        and tensor.ptype == Ptype.arithmetic
        and tensor.dim() == 2
    )


def _grouped_shared_left_matmuls(left, grouped_args):
    """Runs pasted-style same-left grouped matmuls without cross-stage tokens."""
    # STRUCTURAL-REUSE: same-left grouping only helps when there are at least
    # two actual outputs. A single matmul should stay on the plain path.
    if len(grouped_args) <= 1:
        return _regular_grouped_matmuls(left, grouped_args)

    if not _private_2d_arithmetic_tensor(left):
        return _regular_grouped_matmuls(left, grouped_args)
    for right, _ in grouped_args:
        if not _private_2d_arithmetic_tensor(right):
            return _regular_grouped_matmuls(left, grouped_args)

    beaver_protocol = _get_structural_beaver_protocol()
    if not hasattr(beaver_protocol, "shared_left_matmul_group"):
        return _regular_grouped_matmuls(left, grouped_args)

    raw_results = beaver_protocol.shared_left_matmul_group(
        left._tensor,
        [(right._tensor, transpose_left) for right, transpose_left in grouped_args],
    )

    results = []
    left_t = left.t()
    for (right, transpose_left), raw_result in zip(grouped_args, raw_results):
        left_view = left_t if transpose_left else left
        results.append(_wrap_raw_matmul_result(left_view, right, raw_result))
    return results


def _grouped_shared_left_matmuls_with_token(left, grouped_args, reserve_right_specs=None):
    """Runs pasted-style grouped matmuls and returns a forward-left token."""
    # STRUCTURAL-FB-REUSE: this is the only place that asks Beaver to keep
    # x-a / h-a state for later backward use; no tag or global anchor is used.
    if reserve_right_specs is None:
        reserve_right_specs = []

    # STRUCTURAL-REUSE: avoid paying grouped-call overhead for a single actual
    # matmul unless the caller also needs future reserved triples from a token.
    if len(grouped_args) <= 1 and len(reserve_right_specs) == 0:
        return _regular_grouped_matmuls(left, grouped_args), None
    ...
    try:
        raw_results, left_token = beaver_protocol.shared_left_matmul_group(
            left._tensor,
            [(right._tensor, transpose_left) for right, transpose_left in grouped_args],
            return_left_token=True,
            reserve_right_specs=reserve_right_specs,
        )
    except TypeError:
        return _regular_grouped_matmuls(left, grouped_args), None
    ...
    for (right, transpose_left), raw_result in zip(grouped_args, raw_results):
        left_view = left_t if transpose_left else left
        results.append(_wrap_raw_matmul_result(left_view, right, raw_result))
    return results, left_token


def _matmul_with_left_token(left, right, left_token, transpose_left=False):
    """Computes left(.T) @ right using a saved forward-left token when possible."""
    # STRUCTURAL-FB-REUSE: backward calls this for dA / dB / optional dW. If the
    # provider or shape cannot support tokens, it falls back to regular matmul.
    if left_token is None:
        return (left.t() if transpose_left else left).matmul(right)
    if not _private_2d_arithmetic_tensor(left) or not _private_2d_arithmetic_tensor(right):
        return (left.t() if transpose_left else left).matmul(right)

    beaver_protocol = _get_structural_beaver_protocol()
    if not hasattr(beaver_protocol, "matmul_with_left_token"):
        return (left.t() if transpose_left else left).matmul(right)

    raw_result = beaver_protocol.matmul_with_left_token(
        left._tensor,
        right._tensor,
        left_token,
        transpose_left=transpose_left,
    )
    left_view = left.t() if transpose_left else left
    return _wrap_raw_matmul_result(left_view, right, raw_result)
...
class AutogradTake(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        grad = grad_output.new(torch.zeros(size, device=grad_output.device))
        if dim is None:
            ...
            flat_index = index.flatten()
            if index.numel() == 1:
                # Scalar index (0-d or length-1) removes the dim in forward.
                grad.index_add_(dim, flat_index, grad_output.unsqueeze(dim))
            else:
                grad_output_flat = grad_output.flatten(
...
class AutogradIndexSelect(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output.new(torch.zeros(size, device=grad_output.device)).index_add_(dim, index, grad_output)
...
class AutogradGather(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        size, dim, index = ctx.saved_tensors
        return grad_output.new(torch.zeros(size, device=grad_output.device)).scatter_add_(dim, index, grad_output)
...
class AutogradScatter(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        mask = torch.ones(size, device=grad_output.device).scatter(dim, index, torch.zeros(size, device=grad_output.device)).long()
...
class AutogradHardtanh(AutogradFunction):
    ...
    def forward(ctx, input, min_val=-1, max_val=1):
        ...
        if min_val == max_val:
            grad = input.new(torch.zeros(input.size(), device=input.device))
...
class AutogradMatMul(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        # Compute gradients
        # STRUCTURAL-REUSE: generic matmul stays identical to pasted code.
        # LoRA-specific grouped reuse is isolated in AutogradLoRALinear.
...
@register_function("lora_linear")
class AutogradLoRALinear(AutogradFunction):
    @staticmethod
    def forward(ctx, input, weight, lora_A, lora_B, scaling, bias=None):
        # STRUCTURAL-FB-REUSE: compute the two forward matmuls with one shared
        # input opening and reserve the input.T @ grad_lora_hidden triple that
        # backward needs for grad_lora_A.
        _log_capability_once()
        ...
        lora_rank = lora_A.size(0)

        input_2d = input.reshape(-1, in_features)
        reserve_right_specs = []
        if getattr(lora_A, "requires_grad", False):
            reserve_right_specs.append(
                {
                    "right_size": torch.Size((input_2d.size(0), lora_rank)),
                    "transpose_left": True,
                }
            )

        forward_grouped_args = [(weight.t(), False), (lora_A.t(), False)]
        if reserve_right_specs:
            grouped_outputs, input_left_token = _grouped_shared_left_matmuls_with_token(
                input_2d,
                forward_grouped_args,
                reserve_right_specs=reserve_right_specs,
            )
        else:
            # STRUCTURAL-FB-REUSE: eval / frozen-LoRA forward has no backward
            # consumer, so avoid creating an unused left token.
            grouped_outputs = _grouped_shared_left_matmuls(input_2d, forward_grouped_args)
            input_left_token = None
        output = grouped_outputs[0].reshape(
            input_size[:-1] + torch.Size((out_features,))
        )
        lora_hidden = grouped_outputs[1].reshape(
            input_size[:-1] + torch.Size((lora_rank,))
        )
        ctx.save_multiple_for_backward(
            [input, weight, lora_A, lora_B, bias, scaling, lora_hidden, input_left_token]
        )
    ...
    @staticmethod
    def backward(ctx, grad_output):
        input, weight, lora_A, lora_B, bias, scaling, lora_hidden, input_left_token = ctx.saved_tensors
        ...
        if input_requires_grad:
            ...
            if grad_lora_hidden_2d is not None:
                grad_input = grad_input.add(grad_lora_hidden_2d.matmul(lora_A).reshape(input_size))

        if lora_A_requires_grad:
            if grad_lora_hidden_2d is None:
                raise RuntimeError("grad_lora_hidden_2d is required to compute grad_lora_A")
            grad_lora_A_t = _matmul_with_left_token(
                input_2d,
                grad_lora_hidden_2d,
                input_left_token,
                transpose_left=True,
            )
            grad_lora_A = grad_lora_A_t.t()
...
class AutogradPolynomial(AutogradFunction):
    ...
    def forward(ctx, input, coeffs, func="mul"):
        ...
        if isinstance(coeffs, (list, tuple)):
            coeffs = torch.tensor(coeffs, device=input.device)
    ...
    def backward(ctx, grad_output):
        ...
        coeffs *= torch.arange(coeffs.size(0), device=grad_output.device).add(1)
...
class AutogradCosineSimilarity(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        if len(ctx.saved_tensors) == 2:
            ...
            zeros = torch.zeros(grad_output.size(), device=grad_output.device).unsqueeze(dim)
...
class AutogradTrace(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        (size,) = ctx.saved_tensors
        return grad_output.new(torch.eye(size, device=grad_output.device)).mul_(grad_output)
...
class AutogradMean(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        nelement = float(
            input_size[dim] if isinstance(dim, int) or isinstance(dim, slice) else 
            reduce(lambda x, y: x * y, [input_size[i] for i in dim]) if isinstance(dim, list)
            else reduce(lambda x, y: x * y, input_size) 
        )
        if not keepdim and (isinstance(dim, int) or isinstance(dim, slice)):
            grad_output = grad_output.unsqueeze(dim)
        elif isinstance(dim, list):
            for _dim in dim:
                grad_output = grad_output.unsqueeze(_dim)
...
class AutogradVariance(AutogradFunction):
    ...
    def forward(ctx, self, *args, **kwargs):
        ...
        # PASTED-CODE: keep the variance divisor behavior from the pasted
        # gradients.py. This is not part of the reuse port.
        if unbiased:
            divisor -= 1
...
class AutogradSoftmax(AutogradFunction):
    ...
    def backward(ctx, grad_output):
        ...
        if grad_output.dim() == 0 or grad_output.size(dim) == 1:
            return grad_output.new(torch.zeros(grad_output.size(), device=grad_output.device))
