...
# STRUCTURAL-REUSE: this standalone pasted variant intentionally does not use
# local reuse_context tags. Reuse is expressed by explicit grouped matmul calls
# in gradients_pasted_reuse.AutogradLoRALinear.
...
class Gemm(Module):
    ...
    def forward(self, x):
        ...
        if self.trans_b:
            b = b.t()
        # STRUCTURAL-REUSE: no generic tagging here. Only LoRA's fused autograd
        # path performs explicit grouped matmuls.
...
class Linear(Module):
    ...
    def forward(self, x):
        # STRUCTURAL-REUSE: keep normal Linear unchanged; LoRA reuse is handled
        # by the explicit lora_linear autograd function.
...
class LoRALinear(Module):
    ...
    Formula:
        y = x W^T + b + (alpha / r) * ((dropout(x) A^T) B^T)
    """
    ...
    def __init__(
        ...
        if base_weight is None or (bias and base_bias is None):
            pt = torch.nn.Linear(in_features, out_features, bias=bias)
            if base_weight is None:
                base_weight = pt.weight
            if bias and base_bias is None:
                base_bias = pt.bias

        self.register_buffer("weight", base_weight)
        if bias:
            assert base_bias is not None, "base_bias must be provided when bias=True"
            self.register_buffer("bias", base_bias)
    ...
    def forward(self, x):
        ...
        x_d = self.dropout(x) if hasattr(self, "dropout") else x
        lora = x_d.matmul(self.lora_A.t()).matmul(self.lora_B.t())
        out = out.add(lora.mul(self.scaling))
        return out


class MPCLoRALinear(Module):
    """LoRA linear layer with an explicit CrypTen backward implementation."""
    ...
    def __init__(
        ...
        if float(dropout) != 0.0:
            raise NotImplementedError(
                "MPCLoRALinear currently only supports dropout=0.0 so its backward remains fully manual."
            )

        if base_weight is None or (bias and base_bias is None):
            pytorch_module = torch.nn.Linear(in_features, out_features, bias=bias)
            if base_weight is None:
                base_weight = pytorch_module.weight
            if bias and base_bias is None:
                base_bias = pytorch_module.bias
    ...
    def forward(self, x):
        ...
        if isinstance(x, crypten.CrypTensor):
            # STRUCTURAL-REUSE: the fused op directly groups the LoRA matmuls;
            # no external tag context is required.
            return x.lora_linear(self.weight, self.lora_A, self.lora_B, self.scaling, bias)

        out = x.matmul(self.weight.t())
        if bias is not None:
            out = out.add(bias)
        lora = x.matmul(self.lora_A.t()).matmul(self.lora_B.t())
        return out.add(lora.mul(self.scaling))
...
class MatMul(Module):
    ...
    def forward(self, x):
        if hasattr(self, "weight"):
            # STRUCTURAL-REUSE: no generic MatMul tags in this pasted variant.
